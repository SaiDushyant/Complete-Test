...
async def run_crawler_workflow(
    ...
    effective_start_url = start_url or BASELINE_URL
    auth_pages = get_trading_auth_pages(effective_start_url)

    print()
    print("=" * 80)
    print("PLAYWRIGHT APPLICATION BASELINE CRAWLER (5-VIEWPORT REGRESSION TESTING)")
    print("=" * 80)
    print(f"Baseline URL      : {effective_start_url}")
    ...
    async with async_playwright() as playwright:
        ...
        try:
            # 1. Crawl public auth entry pages unauthenticated across all viewports
            print("\n--- Crawling Public Auth Pages (Unauthenticated) ---")
            ...
            # 2. Establish/verify authenticated session state once
            print("\n--- Initializing / Verifying Authenticated Session ---")
            auth_context = await create_authenticated_context(
                browser=browser,
                base_url=effective_start_url,
                email=BASELINE_TEST_USER_EMAIL,
                password=BASELINE_TEST_USER_PASSWORD,
                post_login_url_pattern=BASELINE_POST_LOGIN_URL_PATTERN,
                auth_state_path=BASELINE_AUTH_STATE,
            )
            await auth_context.close()

            # 3. Crawl authenticated application concurrently across viewports with worker pool
            print(f"\n--- Starting Concurrent Crawling across {len(selected_viewports)} Viewports ({workers} workers) ---")
            sem = asyncio.Semaphore(max(1, workers))

            async def crawl_single_viewport(vp: str):
                async with sem:
                    vp_size = get_viewport_config(vp)
                    print(f"\n[Worker] STARTING CRAWL FOR VIEWPORT: {vp.upper()} ({vp_size['width']}x{vp_size['height']})")

                    context = await browser.new_context(
                        storage_state=str(BASELINE_AUTH_STATE),
                        viewport=vp_size,
                    )
            ...
            results = await asyncio.gather(*(crawl_single_viewport(vp) for vp in selected_viewports))
            for vp, stats in results:
                per_viewport_stats[vp] = stats

        except Exception as e:
            print(f"\nCRITICAL CRAWLER ERROR: {e}", file=sys.stderr)
            await browser.close()
            sys.exit(1)
    ...
    print()
    print("=" * 80)
    print("5-VIEWPORT CRAWLING SUMMARY")
    print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(description="Playwright 5-Viewport Application Crawler")
    parser.add_argument(
        "--start-url",
        "--baseline-url",
        dest="start_url",
        ...
        help=f"Baseline start URL to crawl (default: {BASELINE_URL})",
    )
    parser.add_argument(
        "--viewports",
        type=str,
    ...
    parser.add_argument(
        ...
        help=f"Output directory for JSON element snapshots (default: {OUTPUT_DIR})",
    )
    parser.add_argument(
        "--headed",
        action="store_true",
